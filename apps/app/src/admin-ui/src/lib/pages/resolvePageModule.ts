...
// Discover Astro page modules that export `staticPageDefinition`
...
const astroModuleMap = new Map<
  string,
  () => Promise<{ staticPageDefinition?: PageModule }>
>();
...
for (const [path, loader] of Object.entries(astroPageModules)) {
  ...
  if (match) {
    astroModuleMap.set(
      match[1],
      loader as () => Promise<{ staticPageDefinition?: PageModule }>
    );
    continue;
  }
