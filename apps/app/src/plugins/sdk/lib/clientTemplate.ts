...
`
  ...
  return `${typeExports}export class NomosApiError extends Error {
  status;
  code;
  details;
  payload;
...
function createHttpClient(config) {
  ...
  async function request(options) {
    const { method, path, headers = {}, credentials, body, params } = options;
...
export function createNomosClient(opts) {
  ...
  const client = createHttpClient({ baseUrl, fetch: fetchImpl });
